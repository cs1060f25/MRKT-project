-- ============================================================================
-- Fix rpc_mark_ticket_delivered to use TEXT user IDs
-- ============================================================================
-- The function was using UUID types for v_winner_id and v_seller_id,
-- but these columns were changed to TEXT to support Clerk user IDs.
...
AS $$
DECLARE
  v_winner_id text;  -- Changed from uuid to text
...
BEGIN
  ...
  IF auth.uid()::text = v_winner_id OR auth.uid()::text = v_seller_id THEN
